...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(n):
            window = list(itertools.chain.from_iterable(sentence[i:i+2*window_size+1]))
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', (Config.n_window_features * Config.embed_size, \
            Config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
