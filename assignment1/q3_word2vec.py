...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    assert len(gradPred) == len(predicted)
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    u_k = outputVectors[indices[1:],:]
    cost = -np.log(sigmoid(np.dot(u_o, predicted))) \
        - np.sum(np.log(sigmoid(-np.dot(u_k, predicted))))
    gradPred = (sigmoid(np.dot(u_o, predicted)) - 1) * u_o \
        - np.sum((sigmoid(-np.dot(u_k, predicted.reshape((len(predicted),1)))) - 1) * u_k, axis=0)
    tmp = np.dot(outputVectors[indices,:], predicted)
    tmp[1:,:] *= -1
    tmp = sigmoid(tmp) - 1
    tmp[1:,:] *= -1
    grad = tmp * outputVectors[indices,:]
    print grad
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for w in contextWords:
        ...
        cost_, gradPred_, grad_ = word2vecCostAndGradient(
